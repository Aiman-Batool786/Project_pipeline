...
def parse_pdp_response(text: str) -> dict | None:
    try:
        ...
        # ── Description fallback: DESCRIPTION_PC in API JSON ───────────────
        ...
        # ── Description fallback: deep search in API JSON ──────────────────
        if not extracted.get('description'):
            try:
...
# ─────────────────────────────────────────────────────────────────────────────
# DESCRIPTION EXTRACTION — MULTI-STRATEGY
# ─────────────────────────────────────────────────────────────────────────────
    ...
    try:
        with sync_playwright() as p:
            ...
            def handle_response(response):
                ...
                # Existing PDP interception (unchanged)
                ...
                # NEW: Intercept description API calls
                if any(kw in url_r.lower() for kw in [
                    ...
                    try:
                        ...
                        if len(body) > 100:
                            text = body.decode('utf-8', errors='replace')
                            # Only keep if it looks like HTML content or JSON with desc
            ...
            # DOM fallbacks for title, images, price (unchanged)
            if not extracted.get('title'):
            ...
            if not extracted.get('price'):
                try:
                    ...
                    pass
# ─────────────────────────────────────────────────────────────
# WINDOW DATA EXTRACTION (NEW)
# ─────────────────────────────────────────────────────────────

def extract_window_data(page) -> dict:
    try:
        data = page.evaluate("""
        () => {
            return {
                ...
                aerData: window.__AER_DATA__ || {}
            }
        }
        """)
        print("[scraper]    ✅ Window data extracted")
        return data or {}
    except Exception as e:
        print(f"[scraper]    ⚠️  Window data extraction failed: {e}")
        return {}


def parse_window_data(window_data: dict) -> dict:
    extracted = {}

    rp   = window_data.get("runParams", {}) or {}
    init = window_data.get("initData", {}) or {}

    # ── Title ─────────────────────────────────────────────
    extracted['title'] = (
        rp.get("titleModule", {}).get("subject") or
        init.get("titleModule", {}).get("subject") or
        ""
    )

    # ── Price ─────────────────────────────────────────────
    extracted['price'] = (
        rp.get("priceModule", {}).get("formatedPrice") or
        rp.get("priceModule", {}).get("formatedActivityPrice") or
        ""
    )

    # ── Images ────────────────────────────────────────────
    images = rp.get("imageModule", {}).get("imagePathList") or []
    for idx, img in enumerate(images[:20], 1):
        if img:
            if img.startswith('//'):
                img = 'https:' + img
    ...
    # ── Specs ─────────────────────────────────────────────
    props = (
        rp.get("PRODUCT_PROP_PC", {}).get("showedProps") or
        init.get("PRODUCT_PROP_PC", {}).get("showedProps") or
        []
    )
    if props:
        mapped, raw = map_props_to_fields(props)
    ...
    # ── Seller (basic, keep your original extractor intact)
    shop = rp.get("SHOP_CARD_PC") or init.get("SHOP_CARD_PC") or {}
    if shop:
        extracted['store_name'] = str(shop.get("storeName", "") or "")
        extracted['seller_level'] = str(shop.get("sellerLevel", "") or "")
        extracted['seller_positive_rate'] = str(shop.get("sellerPositiveRate", "") or "")

        seller_info = shop.get("sellerInfo", {}) or {}
        if seller_info:
            extracted['seller_id'] = str(seller_info.get("adminSeq", "") or "")
            extracted['store_id']  = str(seller_info.get("storeNum", "") or "")
            extracted['store_url'] = str(seller_info.get("storeURL", "") or "")
    ...
    return extracted
            ...
            if not extracted.get('description'):
                ...
                if not extracted.get('description'):
                    ...
                    # Check if new desc API responses came in during scrolling
                    if captured_desc_api:
                        for desc_text in captured_desc_api:
                            cleaned = re.sub(r'<[^>]+>', ' ', desc_text)
                            cleaned = ' '.join(cleaned.split()).strip()
                ...
                # Step E: Scroll to absolute bottom + wait + try one last time
                if not extracted.get('description'):
                    print("[scraper]    📜 Final scroll to absolute bottom...")
                    page.evaluate('window.scrollTo(0, document.body.scrollHeight)')
                    page.wait_for_timeout(3000)

                    # Final check on any new API captures
                    if captured_desc_api:
