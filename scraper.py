...
def extract_price(result: dict) -> str:
    try:
        ...
        if sku_map:
            ...
            p = (first.get('actSkuCalPrice') or first.get('actSkuMultiCurrencyCalPrice') 
                 or first.get('skuCalPrice', ''))
            if p:
                return f"${p}"
    except Exception:
        pass
...
def fetch_description_url(result: dict) -> str:
    try:
        ...
        with urllib.request.urlopen(req, timeout=10) as r:
            ...
            if 'desc.json' in url:
                return _parse_desc_json(json.loads(raw))
            else:
...
def parse_pdp_response(text: str) -> dict | None:
    try:
        ...
        # Seller, Price, Images, Description...
        extracted.update(extract_seller_info(result))
        extracted['price'] = extract_price(result)

        # Images
        images = []
        for key in ['HEADER_IMAGE_PC', 'imageModule']:
            try:
                images = result[key]['imagePathList'] or []
                if images:
                    break
            except:
                pass
...
# ─────────────────────────────────────────────────────────────────────────────
# BROWSER SCRAPING FUNCTION (FIXED INDENTATION)
# ─────────────────────────────────────────────────────────────────────────────
def _scrape_in_thread(url: str) -> dict:
    """Runs Camoufox in a separate thread."""
    captured_pdp = []
    html = ''
    ...
    try:
        with Camoufox(headless=True, geoip=True, os='windows') as browser:
            ...
            def handle_response(response):
                try:
                    url_r = response.url
                    if 'mtop.aliexpress.pdp.pc.query' in url_r or 'mtop.aliexpress.itemdetail' in url_r:
            ...
            for i in range(30):
                ...
                good = [t for t in captured_pdp if 'PRODUCT_PROP_PC' in t or 'SHOP_CARD_PC' in t or len(t) > 50000]
                if good:
...
def get_product_info(url: str) -> dict | None:
    ...
    defaults = {
        'description': '', 'brand': '', 'color': '', 'dimensions': '', 'weight': '',
        'material': '', 'certifications': '', 'country_of_origin': '', 'warranty': '',
        'product_type': '', 'price': '', 'store_name': '', 'seller_country': ''
    }
    ...
    print(f'\n[scraper] ✅ Extraction completed for: {extracted.get("title")[:70]}...')
    return extracted
