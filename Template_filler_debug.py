...
class TemplateFiller:
    ...
    def get_header_row(self, header_row_num=1):
        """Get the header row with column names
        
        Args:
            header_row_num: Row number containing headers (default: 1 for French template)
        """
        headers = {}
        ...
        for col in range(1, self.ws.max_column + 1):
            ...
            if cell_value:
                # Clean up the header name (remove asterisks, extra spaces)
    ...
    def fill_product_data(self, mapped_data, data_row_num=11):
        ...
        try:
            ...
            print(f"[template] ✅ Filled {filled_count} fields in row {data_row_num}")
            if missing_count > 0:
                print(f"[template] ⚠️  {missing_count} fields not found in template headers")
